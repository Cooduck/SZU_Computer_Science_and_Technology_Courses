...
class AStar:
    def __init__(self, walls):
        self.walls = walls  # 墙的网格（True表示墙，False表示自由空间）
    ...
    def getNeighbors(self, pos):
        ...
        for dx, dy in directions:
            nx, ny = x + dx, y + dy
            if not self.walls[nx][ny]:  # 确保邻居不是墙
                neighbors.append((nx, ny))

        return neighbors

    def aStarSearch(self, start, goal, ghostPos=None):
        """
        执行A*搜索以找到从起点到目标点的最短路径。
        返回路径（从起点到目标点的位置列表），如果没有找到路径则返回None。
        """

        # 防止goal的 x、y 为 float 类型产生报错
        goal = (int(goal[0]), int(goal[1]))

        # 在吃豆人寻豆子的路上，鬼也是一种阻塞
        if ghostPos:
            for pos in ghostPos:
                self.walls[int(pos[0])][int(pos[1])] = False

        # 开放列表是一个优先队列（最小堆），其中的每个元素是一个元组（f_score, pos）
        open_list = []
        heapq.heappush(open_list, (0 + self.manhattanDistance(start, goal), 0, start))  # （f_score, g_score, pos）
        ...
        while open_list:
            ...
            for neighbor in self.getNeighbors(current):
                if neighbor in closed_list:
                    continue  # 如果邻居已经被访问过，则忽略

                tentative_g_score = g + 1  # 每次移动的成本为1

                if neighbor not in g_score or tentative_g_score < g_score[neighbor]:
                    # 如果这条到邻居的路径更好，记录下来
                    came_from[neighbor] = current
                    g_score[neighbor] = tentative_g_score
                    f_score = tentative_g_score + self.manhattanDistance(neighbor, goal)
                    heapq.heappush(open_list, (f_score, tentative_g_score, neighbor))

        # 如果我们退出循环而没有找到目标点，返回None
        return None