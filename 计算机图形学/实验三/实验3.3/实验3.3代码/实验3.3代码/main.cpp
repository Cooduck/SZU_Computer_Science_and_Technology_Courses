...
void init()
{
	...
	light->setAmbient(glm::vec4(0.24725f, 0.1995f, 0.0745f, 1.0f)); // 环境光
	light->setDiffuse(glm::vec4(0.75164f, 0.60648f, 0.22648f, 1.0f)); // 漫反射
	light->setSpecular(glm::vec4(0.628281f, 0.555802f, 0.366065f, 1.0f)); // 镜面反射
	...
	// 设置材质
	mesh->setAmbient(glm::vec4(0.24725, 0.1995, 0.0745, 1.0f)); // 环境光
	mesh->setDiffuse(glm::vec4(0.75164, 0.60648, 0.22648, 1.0f)); // 漫反射
	mesh->setSpecular(glm::vec4(0.628281, 0.555802, 0.366065, 1.0f)); // 镜面反射
	mesh->setShininess(0.4); //高光系数
...
void mainWindow_key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	else if(key == GLFW_KEY_0 && action == GLFW_PRESS && mode == 0x0000) 
	{
		...
		tmp = shininess;
		shininess = std::min(tmp + 1.0, 200.0);
...
int main(int argc, char **argv)
{
	...
	GLFWwindow* mainwindow = glfwCreateWindow(600, 600, "实验3.3", NULL, NULL);
	if (mainwindow == NULL)
	{
