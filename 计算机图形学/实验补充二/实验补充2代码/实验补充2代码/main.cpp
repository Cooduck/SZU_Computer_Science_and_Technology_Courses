...
struct treenode
{
	int nodeId;
   	glm::vec3 translateM;
	glm::vec3 rotateAxis;
   	void (*f)(glm::mat4 modelMatrix);
   	struct treenode *child;
	struct treenode *sibling;

	treenode(int nodeId, glm::vec3 translateM, glm::vec3 rotateAxis, void (*f)(glm::mat4 modelMatrix), treenode *child, treenode *sibling){
		this->nodeId = nodeId;
...
void traverse(MatrixStack &mstack, treenode *node, glm::mat4 modelMatrix)
{
	...
	modelMatrix = glm::translate(modelMatrix, node->translateM);
	modelMatrix = glm::rotate(modelMatrix, glm::radians(robot.theta[node->nodeId]), node->rotateAxis);
	node->f(modelMatrix);
	if(node->child != NULL) traverse(mstack, node->child, modelMatrix);
	modelMatrix = mstack.pop();
	if(node->sibling != NULL) traverse(mstack, node->sibling, modelMatrix);
}
...
void display()
{
	...
	// treenode* rightupleg = new treenode(robot.RightUpperLeg, glm::vec3(0.5 * robot.TORSO_WIDTH - 0.5 * robot.UPPER_LEG_WIDTH, 0.0, 0.0), glm::vec3(0.0, 0.0, 1.0), right_upper_leg, NULL, NULL);
	// treenode* leftupleg = new treenode(robot.LeftUpperLeg, glm::vec3(-0.5 * robot.TORSO_WIDTH + 0.5 * robot.UPPER_LEG_WIDTH, 0.0, 0.0), glm::vec3(0.0, 0.0, 1.0), left_upper_leg, NULL, rightupleg);
	// treenode* rightuparm = new treenode(robot.RightUpperArm, glm::vec3(0.5 * robot.TORSO_WIDTH + 0.5 * robot.UPPER_ARM_WIDTH, robot.TORSO_HEIGHT, 0.0), glm::vec3(0.0, 0.0, 1.0), right_upper_arm, NULL, leftupleg);
	// treenode* leftuparm = new treenode(robot.LeftUpperArm, glm::vec3(-0.5 * robot.TORSO_WIDTH - 0.5 * robot.UPPER_ARM_WIDTH, robot.TORSO_HEIGHT, 0.0), glm::vec3(0.0, 0.0, 1.0), left_upper_arm, NULL, rightuparm);
	// treenode* robothead = new treenode(robot.Head, glm::vec3(0.0, robot.TORSO_HEIGHT, 0.0), glm::vec3(0.0, 1.0, 0.0), head, NULL, leftuparm);
	// treenode* robotbody = new treenode(robot.Torso, glm::vec3(0.0, 0.0, 0.0), glm::vec3(0.0, 1.0, 0.0), torso, robothead, NULL);

	// traverse(mstack, robotbody, modelMatrix);
    ...
    // @TODO: Task-2 左小臂
	mstack.push(modelMatrix);	// 保存左大臂变换矩阵
	...
	modelMatrix = mstack.pop();	// 恢复左大臂变换矩阵
	modelMatrix = mstack.pop();	// 恢复躯干变换矩阵
    ...
    // @TODO: Task-2 右小臂
	mstack.push(modelMatrix);	// 保存右大臂变换矩阵
	modelMatrix = glm::translate(modelMatrix, glm::vec3(0.0, -robot.UPPER_ARM_HEIGHT, 0.0));
	...
	modelMatrix = mstack.pop();	// 恢复右大臂变换矩阵
	modelMatrix = mstack.pop();	// 恢复躯干变换矩阵
	...
	// @TODO: Task-2 左小腿
	mstack.push(modelMatrix);	// 保存左大腿变换矩阵
	...
	modelMatrix = mstack.pop();	// 恢复左大腿变换矩阵
	modelMatrix = mstack.pop();	// 恢复躯干变换矩阵
    ...
    // @TODO: Task-2 右小腿
	mstack.push(modelMatrix);	// 保存右大腿变换矩阵
	modelMatrix = glm::translate(modelMatrix, glm::vec3(0.0, -robot.UPPER_LEG_HEIGHT, 0.0));
	...
	modelMatrix = mstack.pop();	// 恢复右大腿变换矩阵
	modelMatrix = mstack.pop();	// 恢复躯干变换矩阵
}
...
int main(int argc, char **argv)
{
	...
	// 配置窗口属性
	GLFWwindow* window = glfwCreateWindow(600, 600, "实验补充2", NULL, NULL);
	if (window == NULL)
	{
