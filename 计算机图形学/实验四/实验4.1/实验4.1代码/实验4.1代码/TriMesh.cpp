...
void TriMesh::generateDisk(int num_division, float radius)
{
    ...
    // @TODO: Task2 请在此添加代码
    int num_samples = num_division;  // 设置分割数，即圆盘切片数
    float step = 2 * M_PI / num_samples;  // 每个切片的弧度（角度增量）

    // 计算圆盘边缘的顶点坐标
    for (int i = 0; i < num_samples; i++)
    {
        float r_r_r = i * step;  // 当前切片的角度
        float x = radius * cos(r_r_r);  // 计算圆盘上该点的x坐标
        float y = radius * sin(r_r_r);  // 计算圆盘上该点的y坐标
        // 将计算出的顶点坐标添加到顶点列表
        vertex_positions.push_back(glm::vec3(x, y, 0.0));
        // 计算并添加顶点法向量
        vertex_normals.push_back(normalize(glm::vec3(x, y, 1.0)));
        // 添加顶点颜色
        vertex_colors.push_back(normalize(glm::vec3(x, y, 1.0)));
    }
    
    // 添加圆盘中心点的顶点数据
    vertex_positions.push_back(glm::vec3(0.0, 0.0, 0.0));  // 中心点的坐标
    vertex_normals.push_back(normalize(glm::vec3(0.0, 0.0, 1.0)));  // 法向量
    vertex_colors.push_back(normalize(glm::vec3(0.0, 0.0, 1.0)));  // 颜色

    glm::vec2 scale(0.5, 0.5);  // 纹理坐标的缩放因子
    glm::vec2 delta(0.5, 0.5);  // 纹理坐标的偏移量

    // 生成三角形面片
    for (int i = 0; i < num_samples; i++)
    {
        // 创建一个面片（每两个相邻的顶点与圆盘中心形成一个三角形）
        faces.push_back(vec3i(i % num_samples, (i + 1) % num_samples, num_samples));

        // 添加纹理坐标：为当前切片的三个顶点分配纹理坐标
        vertex_textures.push_back(glm::vec2(cos(i % num_samples * step), sin(i % num_samples * step)) * scale + delta);
        vertex_textures.push_back(glm::vec2(cos((i + 1) % num_samples * step), sin((i + 1) % num_samples * step)) * scale + delta);
    ...
    // 设置法向量的索引，这里法向量与顶点索引一致，因此直接使用 faces
    normal_index = faces;

    // 设置颜色的索引，颜色与顶点索引一致，因此也直接使用 faces
    color_index = faces;
...
void TriMesh::generateCone(int num_division, float radius, float height)
{
    ...
    // @TODO: Task2 请在此添加代码
    int num_samples = num_division;  // 设置分割数，即圆锥底面切片数
    float step = 2 * M_PI / num_samples;  // 每个切片的弧度（角度增量）

    // 生成圆锥底面的顶点
    float z = -height;  // 圆锥底面的z坐标
    for (int i = 0; i < num_samples; i++)
    {
        float r_r_r = i * step;  // 当前切片的角度
        float x = radius * cos(r_r_r);  // 底面顶点的x坐标
        float y = radius * sin(r_r_r);  // 底面顶点的y坐标
        // 添加圆锥底面顶点坐标
        vertex_positions.push_back(glm::vec3(x, y, z));
        // 计算并添加圆锥底面顶点的法向量
        vertex_normals.push_back(normalize(glm::vec3(x, y, 1.0)));
    ...
    // 添加圆锥顶点（顶端）
    vertex_positions.push_back(glm::vec3(0.0, 0.0, height));  // 圆锥顶端的坐标
    vertex_normals.push_back(normalize(glm::vec3(0.0, 0.0, 1.0)));  // 顶端法向量
    vertex_colors.push_back(normalize(glm::vec3(0.0, 0.0, 1.0)));  // 顶端颜色

    // 生成三角形面片（圆锥侧面）
    for (int i = 0; i < num_samples; i++)
    {
        // 创建一个三角形面片（每两个相邻的底面顶点和圆锥顶点构成一个面）
        faces.push_back(vec3i(i % num_samples, (i + 1) % num_samples, num_samples));

        // 为当前三角形面片的三个顶点添加纹理坐标
        vertex_textures.push_back(glm::vec2(1.0 * i / num_samples, 0.0));  // 底面第一个顶点的纹理坐标
        vertex_textures.push_back(glm::vec2(1.0 * (i + 1) / num_samples, 0.0));  // 底面第二个顶点的纹理坐标
        vertex_textures.push_back(glm::vec2(0.5, 1));  // 顶点的纹理坐标（圆锥顶点）
