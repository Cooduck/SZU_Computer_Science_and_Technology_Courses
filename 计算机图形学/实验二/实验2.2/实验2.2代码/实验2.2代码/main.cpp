...
void read_off(const std::string filename)
{
	...
	else
	{
		...
		for(int i = 0; i < nVertices; ++i){
			double a, b, c;
			fin >> a >> b >> c;
			vertices.emplace_back(glm::vec3{a, b, c});	// 读入顶点存入vertices
		}
		...
		for(int i = 0; i < nFaces; ++i){
			int tmp, a, b, c;
			fin >> tmp >> a >> b >> c;
			faces.emplace_back(vec3i{a, b, c});	// 读入三角面片存入faces
		}
	}
	fin.close();
}
...
void storeFacesPoints()
{
	...
	// 读取每个三角面片，将三角面片的顶点存入points，三角面片的每个顶点的颜色均相同，
	// 且每两个连续的三角面片顶点颜色均相同，如此可以保证正方体的每个面为不同的颜色
	int cnt_color = 0;
	for(auto face : faces){
		points.emplace_back(glm::vec3(vertices[face.a]));	// 存入三角面片的第一个顶点
		points.emplace_back(glm::vec3(vertices[face.b]));	// 存入三角面片的第二个顶点
		points.emplace_back(glm::vec3(vertices[face.c]));	// 存入三角面片的第三个顶点
		colors.emplace_back(vertex_colors[cnt_color/2%8]);	// 设置三角面片第一个顶点的颜色
		colors.emplace_back(vertex_colors[cnt_color/2%8]);	// 设置三角面片第二个顶点的颜色
		colors.emplace_back(vertex_colors[cnt_color/2%8]);	// 设置三角面片第三个顶点的颜色
		++cnt_color;
	}
}
...
void display(void)
{
	...
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);	// 清理窗口的颜色缓存和深度缓存
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	else if (key == GLFW_KEY_4 && action == GLFW_PRESS && mode == 0x0000)
	{
		...
		glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);	// 启用线绘制模式
	}
	// @TODO: Task5:关闭线绘制模式
	else if (key == GLFW_KEY_4 && action == GLFW_PRESS && mode == GLFW_MOD_SHIFT)
	{
		cout << "line mode: disable" << endl;	
		glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);	// 关闭线绘制模式，恢复为原本的填充绘制模式
	}
}
...
int main(int argc, char** argv)
{
	...
	// 配置窗口属性
	GLFWwindow* window = glfwCreateWindow(600, 600, "实验2.2", NULL, NULL);
	if (window == NULL)
	{
