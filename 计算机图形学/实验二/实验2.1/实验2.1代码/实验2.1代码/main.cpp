...
void mouse_button_callback(GLFWwindow *window, int button, int action, int mode)
{
	...
	if(button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS){
		WindowSquareColor = GREEN;	// 点击鼠标左键，WindowSquareColor改为绿色
	}
	else if(button == GLFW_MOUSE_BUTTON_RIGHT && action == GLFW_PRESS){
		WindowSquareColor = YELLOW;	// 点击鼠标右键，WindowSquareColor改为黄色
	}
}
void key_callback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
	...
	if(key == GLFW_KEY_ESCAPE && action == GLFW_PRESS){
		glfwSetWindowShouldClose(window, GL_TRUE);	// 点击键盘Esc，退出窗口
	}
	else if(key == GLFW_KEY_R && action == GLFW_PRESS){
		WindowSquareColor = RED;	// 点击键盘R，WindowSquareColor改为红色
	}
	else if(key == GLFW_KEY_B && action == GLFW_PRESS){
		WindowSquareColor = BLUE;	// 点击键盘B，WindowSquareColor改为蓝色
	}
	else if(key == GLFW_KEY_W && action == GLFW_PRESS){
		WindowSquareColor = WHITE;	// 点击键盘W，WindowSquareColor改为白色
	}
}
void sroll_callback(GLFWwindow *window, double xoffset, double yoffset)
{
	// @TODO: 课堂练习#2 控制正方形的旋转动画
	// 鼠标滚轮上滑，正方形顶点角度偏移为负，相当于正方体顺时针旋转
	// 鼠标滚轮下滑，正方形顶点角度偏移为正，相当于正方体逆时针旋转
...
int main(int argc, char **argv)
{
	...
	// 创建窗口。
	GLFWwindow *window = glfwCreateWindow(width, height, "实验2.1", NULL, NULL);
